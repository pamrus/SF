...
Boost_Asio_client::Boost_Asio_client(boost::asio::io_service* ioService): 
    _ioService(ioService), _socket(*_ioService), _started(true), _timer(*_ioService), _logger(client_logname) { }
...
void Boost_Asio_client::onLogin(const std::string& msg)   // Реакция на успешный вход
{
    ...
    if (answer == "ok")
    {
        ...
        _login = userLogin;
        _name = userName;
        _logger << "SYS: " + _login + " logged in.";
        doAskMessages();
    }
    else
    {
        std::cout << "Ошибка входа!" << std::endl;
        _logger << "SYS: failed to log in.";
        boost::system::error_code ec;
        ec.assign(errno, boost::system::generic_category());
        onConnect(ec);
    }
...
void Boost_Asio_client::doSendMessage()
{
    ...
    _logger << "OUT: <" + messageReceiver + "> " + messageText;
    doWrite(msg);
    doSendMessage();
}
