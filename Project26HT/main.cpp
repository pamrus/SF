...
constexpr size_t memoryUsage = 1000; // Объём памяти под данные, МБ
...
int main()
{
	...
	randomVector** dataVectors = new randomVector*[threadsCount];
	std::vector<std::thread> threads;

	std::cout << "Генерация начата...";
	for (unsigned int i = 0; i < threadsCount; ++i)
		threads.push_back(std::thread( [&dataVectors, threadsCount, i]() mutable { dataVectors[i] = new randomVector(vectorSize / threadsCount, randMean, randDeviation); } ) );
	std::for_each(threads.begin(), threads.end(), [](std::thread& t) { t.join(); });
	std::cout << "выполнено!" << std::endl;
	threads.clear();

	for (unsigned int i = 0; i < threadsCount; ++i)
		threads.push_back(std::thread(std::ref(*dataVectors[i]), "[" + std::to_string(i) + "]"));
	std::for_each(threads.begin(), threads.end(), [](std::thread& t) { t.join(); });
	threads.clear();
