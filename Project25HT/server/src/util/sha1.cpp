...
{
    Block shaResult = sha1(message, msize_bytes);
    ...
    for (int i = 0; i < 5; ++i)
    {
        ss << ((shaResult[i] & 0xF0000000) >> 28)
            << ((shaResult[i] & 0x0F000000) >> 24)
            << ((shaResult[i] & 0x00F00000) >> 20)
            << ((shaResult[i] & 0x000F0000) >> 16)
            << ((shaResult[i] & 0x0000F000) >> 12)
            << ((shaResult[i] & 0x00000F00) >> 8)
            << ((shaResult[i] & 0x000000F0) >> 4)
            << ((shaResult[i] & 0x0000000F) >> 0);
    }
    return ss.str();
}
